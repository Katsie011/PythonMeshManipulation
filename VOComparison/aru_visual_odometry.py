...
def predicted_depth_motion_est(depth_f0, img_f0, img_f1, K, det=orb, min_matches=50, vo=pyvo, verify_trans=False,
                               max_step_d=2):
    """
...
def get_depth_vo_path_from_saved(output_directory, dataset, det=orb, start_frame=None, stop_frame=None,
                                 verify_trans=False, max_step_d=2):
    ...
    for i in tqdm.trange(stop_frame):
        d = depth_filenames[i]
        ...
        transforms[i] = predicted_depth_motion_est(depth, img_f0=im0, img_f1=im1, K=K, det=det,
                                                   verify_trans=verify_trans, max_step_d=max_step_d)

    return generate_transform_path(transforms)


def get_vo_path(dataset, start_frame=None, stop_frame=None):
    if start_frame is None:
        start_frame = 0
...
if __name__ == "__main__":
    ...
    x_vo, y_vo = get_vo_path(dataset=dataset, stop_frame=num_frames)

    print("Getting pydnet orb path")
    x_p, y_p = get_depth_vo_path_from_saved(output_directory=output_directory, dataset=dataset, det=orb,
                                            stop_frame=num_frames)

    print("Getting pydnet sift path")
    x_psift, y_psift = get_depth_vo_path_from_saved(output_directory=output_directory, dataset=dataset, det=sift,
                                                    stop_frame=num_frames)
    ...
    im0 = dataset.get_cam0(num_frames)

    axp[0, 0].imshow(cv2.cvtColor(im0, cv2.COLOR_BGR2RGB))
    axp[0, 0].set_title(f"Image at frame {num_frames}")
    axp[0, 0].axis('off')

    depth = np.load(os.path.join(output_directory, depth_filenames[num_frames]))
    sc = axp[0, 1].imshow(depth, 'jet')
    axp[0, 1].set_title(f" Predicted depth at frame {num_frames}")
    axp[0, 1].axis('off')
