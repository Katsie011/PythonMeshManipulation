...
class MonodepthDataloader(object):
    ...
    def __init__(self, data_path, filenames_file, params, dataset, mode):
        ...
        if mode == 'train':
            ...
            left_image  = tf.cond(pred=do_flip > 0.5, true_fn=lambda: tf.image.flip_left_right(right_image_o), false_fn=lambda: left_image_o)
            right_image = tf.cond(pred=do_flip > 0.5, true_fn=lambda: tf.image.flip_left_right(left_image_o),  false_fn=lambda: right_image_o)

            # randomly augment images
            do_augment  = tf.random.uniform([], 0, 1)
            left_image, right_image = tf.cond(pred=do_augment > 0.5, true_fn=lambda: self.augment_image_pair(left_image, right_image), false_fn=lambda: (left_image, right_image))
    ...
    def read_image(self, image_path):
        ...
        image  = tf.cond(pred=file_cond, true_fn=lambda: tf.image.decode_jpeg(tf.io.read_file(image_path)), false_fn=lambda: tf.image.decode_png(tf.io.read_file(image_path)))

        # if the dataset is cityscapes, we crop the last fifth to remove the car hood
        if self.dataset == 'cityscapes':
            o_height    = tf.shape(input=image)[0]
