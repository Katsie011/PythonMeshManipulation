...
def bilinear_sampler_1d_h(input_images, x_offset, wrap_mode='border', name='bilinear_sampler', **kwargs):
    ...
    def _interpolate(im, x, y):
        with tf.compat.v1.variable_scope('_interpolate'):
            ...
            if _wrap_mode == 'border':
                _edge_size = 1
                im = tf.pad(tensor=im, paddings=[[0, 0], [1, 1], [1, 1], [0, 0]], mode='CONSTANT')
                x = x + _edge_size
                y = y + _edge_size
    ...
    with tf.compat.v1.variable_scope(name):
        _num_batch    = tf.shape(input=input_images)[0]
        _height       = tf.shape(input=input_images)[1]
        _width        = tf.shape(input=input_images)[2]
        _num_channels = tf.shape(input=input_images)[3]
