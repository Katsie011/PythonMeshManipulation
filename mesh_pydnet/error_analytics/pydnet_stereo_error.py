...
parser.add_argument('--checkpoint_dir', type=str, help='path to a specific checkpoint to load',
                    ...
                    # default = '/home/kats/Code/training_monodepth/tmp/Husky2v0/Husky2v0/model-100000')
                    # default='/home/kats/Code/training_monodepth/tmp/Husky2v1/Husky2v1/model-117950')
                    # default = '/home/kats/Code/training_monodepth/tmp/Husky2v0/Husky2v0/model-117950')
                    # default = "/home/kats/Code/training_monodepth/tmp/MonodepthHusky2v0_scratch/MonodepthHusky2v0/model-117950")
                    default='/home/kats/Code/aru_sil_py/reconstruction/mesh_pydnet/checkpoints/MtrxGodzilla/PydnetTransferHusky10k/model'
                            '-117950')
...
from reconstruction.HyperParameters import MIN_DISTANCE, MAX_DISTANCE, MAX_NUM_FEATURES_DETECT
sift = img_ft.FeatureDetector(det_type='sift', max_num_ft=MAX_NUM_FEATURES_DETECT)
orb = img_ft.FeatureDetector(det_type='orb', max_num_ft=MAX_NUM_FEATURES_DETECT)
...
def predict_dataset_get_stereo_e(dataset, scaling_factor = 1, plots=True, verbose=False, training_size=(256, 512), max_frames=None,
    ...
    #     return None
    disp_max = dataset.calib.baseline[0] * (dataset.calib.cam0_camera_matrix[0, 0]) / MIN_DISTANCE
    ...
    with tf.Graph().as_default():
        ...
        with tf.compat.v1.Session() as sess:
            ...
            for counter in tqdm.trange(max_frames):
                ...
                disparity = disp[0, :, :, 0].squeeze() * train_width * scaling_factor
                ...
                if aru_core_pts:
                    frame_core_e, frame_core_ratio ,core_pts, Tr_pydnet, Tr_stereo = pydnet_vs_aru_core_pts(
                        prediction=disparity, frame=counter,
                    ...
                    #                                                 return_mse=False,
                    #                                                 use_depth=False)
                    if (Tr_stereo is not None) and (Tr_stereo.shape == (4, 4)):
                        ...
                        print(f"Bad Tr stereo {Tr_stereo}")

                    if (Tr_pydnet is not None) and (Tr_pydnet.shape == (4, 4)):
                        ...
                        print(f"Bad Tr stereo {Tr_pydnet}")
                    ...
                    if plots:
                        # Scaling down the u component for plotting
                        # core_pts[:,0] = core_pts[:,0]*disparity.shape[1]/fullsize_disparity.shape[1]
                        # core_pts[:,2]  = core_pts[:,2]*disparity.shape[1]/fullsize_disparity.shape[1]
                        # core_pts[:,1] = core_pts[:,1]*disparity.shape[0]/fullsize_disparity.shape[0]
                        #sampling predicted disp at points
                        small_disp_sampled_at_core = disparity[core_pts[:, 1].astype(int), core_pts[:, 0].astype(int)]

                        # Sizing down the left image for plotting
                        im_e_core = cv2.cvtColor(cv2.resize(imgl, disparity.shape[::-1]), cv2.COLOR_RGB2BGR)
                        im_disp_core, im_disp_pred = im_e_core.copy(), im_e_core.copy()

                        # sizing the error to uint8
                        ...
                        scaled_s_dis[scaled_s_dis > 255] = 255
                        stereo_disp_colors = cv2.applyColorMap(scaled_s_dis.astype(np.uint8), cv2.COLORMAP_INFERNO)
                        scaled_disp_s = (np.abs(small_disp_sampled_at_core) * 255 / disp_max)
                        scaled_disp_s[scaled_disp_s > 255] = 255
                        pred_disp_colors = cv2.applyColorMap(scaled_disp_s.astype(np.uint8), cv2.COLORMAP_INFERNO)
                ...
                if plots:
                    color_disp = disparity * 255 / disp_max
                    color_disp[color_disp > 255] = 255
                    color_disp = color_disp.astype(np.uint8)

                    if get_sift:
                        img_and_depth = np.hstack((cv2.cvtColor(cv2.resize(im_disp_sift, disparity.shape[::-1]),
                                                                cv2.COLOR_RGB2BGR), cv2.applyColorMap(
                            color_disp, cv2.COLORMAP_INFERNO)))
                    else:
...
if __name__ == "__main__":
    ...
    es1,  [x_core1, y_core1], [x_pydnet1, y_pydnet1] = predict_dataset_get_stereo_e(dataset=dataset_obj,scaling_factor=1,
                                                                               get_sift=False, aru_core_pts=True,
                                                                               get_orb=False, plots=True,
