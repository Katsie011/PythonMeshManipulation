"""
Code credits to: https://github.com/yycho0108/vitamin-e
"""
...
def curvature(img):
    ...
    fx = gradient(img, dx=1, dy=0, **farg)
    fy = gradient(img, dx=0, dy=1, **farg)
    fxx = gradient(img, dx=2, dy=0, **farg)
    fyy = gradient(img, dx=0, dy=2, **farg)
    fxy = gradient(img, dx=1, dy=1, **farg)
